...
def test():
    ...
    deviceList = get_caminf()
    cam = init_cam(deviceList,1)
    GetImage(cam)
    cam_show(cam,'test')
    stop_cam(cam)


def cam_show(cam,name):
    # try:
    ...
    while keyValue != ord('q'):
        try:
            ...
            try:
                ...
                pass      
            cv2.imwrite(path+"/"+str(count)+".png",src)
...
def init_cam(deviceList,nConnectionNum):
...
def GetImage(cam):
    ...
    if None != stOutFrame.pBufAddr and 0 == ret:
        ...
        temp = temp.reshape((stOutFrame.stFrameInfo.nHeight,stOutFrame.stFrameInfo.nWidth))  #注意 这里要改成使用相机的宽和高
